...
// --------------------------------------------------
// ESP-NOW WIRELESS LIMITS
// --------------------------------------------------

bool wirelessHomePressed = false;
bool wirelessEndPressed = false;
...
void onReceive(
  ...
  if (len != sizeof(LimitPacket))
    return;

  memcpy(&packet, data, sizeof(packet));

  if (packet.nodeID == 1)
    wirelessHomePressed = packet.pressed;

  if (packet.nodeID == 2)
    wirelessEndPressed = packet.pressed;
}
...
void loop() {
  ...
  // Toggle wiring is assumed active LOW.
